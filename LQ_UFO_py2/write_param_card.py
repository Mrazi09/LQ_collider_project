...
from io import open
__date__ = u"3 june 2010"
__author__ = u'olivier.mattelaer@uclouvain.be'

class ParamCardWriter(object):
    
    header = \
    u"""######################################################################\n""" + \
    u"""## PARAM_CARD AUTOMATICALY GENERATED BY THE UFO  #####################\n""" + \
    u"""######################################################################\n"""   
    
    def __init__(self, filename, list_of_parameters=None, generic=False):
        u"""write a valid param_card.dat"""
        
        if not list_of_parameters:
            from parameters import all_parameters
            list_of_parameters = [param for param in all_parameters if \
                                                       param.nature==u'external']
    ...
    def order_param(obj1, obj2):
        u""" order parameter of a given block """
    ...
    def write_card(self, all_ext_param):
        ...
        for name in [u'DECAY', u'MASS',u'SMINPUTS']:
    ...
    def write_block(self, name):
        ...
        if name!=u'DECAY':
            self.fsock.write(u"""Block %s \n""" % name)

    def write_param(self, param, lhablock):
        
        lhacode=u' '.join([u'%3s' % key for key in param.lhacode])
        if lhablock != u'DECAY':
    ...
    def write_dep_param_block(self, lhablock):
        ...
        for parameter in all_parameters:
            try:
                exec(u"%s = %s" % (parameter.name, parameter.value))
            except Exception:
                pass
        text = u"##  Not dependent paramater.\n"
        text += u"## Those values should be edited following analytical the \n"
        text += u"## analytical expression. Some generator could simply ignore \n"
        text += u"## those values and use the analytical expression\n"
        ...
        for part, param in data:
            if isinstance(param.value, unicode):
    ...
    data=u"""Block QNUMBERS %(pdg)d  # %(name)s 
    ...
    def write_qnumber(self):
        ...
        text=u"""#===========================================================\n"""
        text += u"""# QUANTUM NUMBERS OF NEW STATE(S) (NON SM PDG CODE)\n"""
        text += u"""#===========================================================\n\n"""
        
        for part in all_particles:
            if part.pdg_code in self.sm_pdg or part.pdg_code < 0:
                continue
            text += self.data % {u'pdg': part.pdg_code,
                                 u'name': part.name,
                                 u'charge': 3 * part.charge,
                                 u'spin': 2 * part.spin + 1,
                                 u'color': part.color,
                                 u'antipart': part.name != part.antiname and 1 or 0}
...
if u'__main__' == __name__:
    ParamCardWriter(u'./param_card.dat', generic=True)
    print u'write ./param_card.dat'
