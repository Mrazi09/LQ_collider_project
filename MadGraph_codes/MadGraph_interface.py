...
#Number of runs
N_max = len(glob.glob("/scratch_p/jpedropino/1_LQCOLLIDER_RUNS/Cards/Cards_1/*"))
N = N_max
...
path_MadGraph = "/home/jpedropino@UA.PT/Workstation/MadGraph/MG5_aMC_v2_7_3_py3"

#path to run folder
path_run = "/scratch_p/jpedropino/1_LQCOLLIDER_RUNS/Events/Events_1"

#path to MadGraph parameter cards
path_ParamCards = "/scratch_p/jpedropino/1_LQCOLLIDER_RUNS/Cards/Cards_1"

#path to save Data
path_Data = "/scratch_p/jpedropino/1_LQCOLLIDER_RUNS/Data_MadGraph"
...
for idx in range(1, N):
    ...
    Card_path_TChannel = "{path}/Events/Events_1/LQ_T_channel_{index}/Cards/Cards_1".format(path = path_MadGraph, index = idx)
    Card_path_GluonSplit = "{path}/Events/Events_1/LQ_GluonSplit_channel_{index}/Cards/Cards_1".format(path = path_MadGraph, index = idx)
    Card_path_GluonSplit_bc = "{path}/Events/Events_1/LQ_GluonSplitBC_channel_{index}/Cards/Cards_1".format(path = path_MadGraph, index = idx)
    Card_path_GluonSplit_bs = "{path}/Events/Events_1/LQ_GluonSplitBS_channel_{index}/Cards/Cards_1".format(path = path_MadGraph, index = idx)
    
    Tchannel = open("./Events/Events_1/F_Tchannel_{}.sh".format(idx), "x")
    ...
    Tchannel.write("generate p p > e1 e1bar $$ a z h \n")
    Tchannel.write("output Events/Events_1/LQ_T_channel_{} \n".format(idx))
    Tchannel.write("exit \n")
    Tchannel.close()
    
    GluonSplit = open("./Events/Events_1/F_GluonSplit_{}.sh".format(idx), "x")
    ...
    GluonSplit.write("generate p p > e1 e1bar d3 d3bar $$ a z h \n")
    GluonSplit.write("output Events/Events_1/LQ_GluonS_channel_{} \n".format(idx))
    GluonSplit.write("exit \n")
    GluonSplit.close()
    
    GluonSplit_BC = open("./Events/Events_1/F_GluonSplit_BC_{}.sh".format(idx), "x")
    ...
    GluonSplit_BC.write("generate p p > e1 e1bar d3 u2bar $$ a z h \n")
    GluonSplit_BC.write("output Events/Events_1/LQ_GluonS_BC_channel_{} \n".format(idx))
    GluonSplit_BC.write("exit \n")
    GluonSplit_BC.close()
    
    GluonSplit_BS = open("./Events/Events_1/F_GluonSplit_BS_{}.sh".format(idx), "x")
    ...
    GluonSplit_BS.write("generate p p > e1 e1bar d3 d2bar $$ a z h \n")
    GluonSplit_BS.write("output Events/Events_1/LQ_GluonS_BS_channel_{} \n".format(idx))
    GluonSplit_BS.write("exit \n")
    GluonSplit_BS.close()

    Tchannel = open("./Events/Events_1/S_Tchannel_{}.sh".format(idx), "x")
    GluonSplit = open("./Events/Events_1/S_GluonSplit_{}.sh".format(idx), "x")
    GluonSplit_BC = open("./Events/Events_1/S_GluonSplit_BC_{}.sh".format(idx), "x")
    GluonSplit_BS = open("./Events/Events_1/S_GluonSplit_BS_{}.sh".format(idx), "x")
    ...
    if Full_chain == True:
        ...
        Tchannel.write("launch Events/Events_1/LQ_T_channel_{} \n".format(idx))
        ...
        GluonSplit.write("launch Events/Events_1/LQ_GluonS_channel_{} \n".format(idx))
        ...
        GluonSplit_BC.write("launch Events/Events_1/LQ_GluonS_BC_channel_{} \n".format(idx))
        ...
        GluonSplit_BS.write("launch Events/Events_1/LQ_GluonS_BS_channel_{} \n".format(idx))
    ...
    Tchannel = open("./Events/Events_1/P_Tchannel_{}.sh".format(idx), "x")
    ...
    Tchannel.write("\n")
    Tchannel.write("cd {path}/Events/Events_1/LQ_T_channel_{index}/bin \n".format(path = path_MadGraph, index = idx))
    Tchannel.write("./cleanall \n")
    Tchannel.write("cd {path}/Events/Events_1/run_01 \n".format(path = path_MadGraph))
    ...
    GluonSplit = open("./Events/Events_1/P_GluonSplit_{}.sh".format(idx), "x")
    GluonSplit.write("cd {} \n".format(path_MadGraph))
    GluonSplit.write("./bin/mg5_aMC {path}/F_GluonSplit_{index}.sh \n".format(path = path_run, index = idx))
    GluonSplit.write("cd {} \n".format(path_MadGraph))
    GluonSplit.write("./bin/mg5_aMC {path}/S_GluonSplit_{index}.sh \n".format(path = path_run, index = idx))
    GluonSplit.write("\n")
    GluonSplit.write("cd {path}/Events/Events_1/LQ_GluonS_channel_{index}/bin \n".format(path = path_MadGraph, index = idx))
    GluonSplit.write("./cleanall \n")
    GluonSplit.write("cd {path}/Events/Events_1/run_01 \n".format(path = path_MadGraph))
    ...
    GluonSplit_BC = open("./Events/Events_1/P_GluonSplit_BC_{}.sh".format(idx), "x")
    ...
    GluonSplit_BC.write("\n")
    GluonSplit_BC.write("cd {path}/Events/Events_1/LQ_GluonS_BC_channel_{index}/bin \n".format(path = path_MadGraph, index = idx))
    GluonSplit_BC.write("./cleanall \n")
    GluonSplit_BC.write("cd {path}/Events/Events_1/run_01 \n".format(path = path_MadGraph))
    ...
    GluonSplit_BS = open("./Events/Events_1/P_GluonSplit_BS_{}.sh".format(idx), "x")
    ...
    GluonSplit_BS.write("\n")
    GluonSplit_BS.write("cd {path}/Events/Events_1/LQ_GluonS_BS_channel_{index}/bin \n".format(path = path_MadGraph, index = idx))
    GluonSplit_BS.write("./cleanall \n")
    GluonSplit_BS.write("cd {path}/Events/Events_1/run_01 \n".format(path = path_MadGraph))
...
for idx in range(1, N):
    ...
    try:
        ...
        #Read run_tag file
        if Cond_TChannel == True:  
            Output_tchannel = open('{path}/Events/Events_1/LQ_T_channel_{index}/Events/run_01/run_01_tag_1_banner.txt'.format(path = path_MadGraph, index = idx), 'r')
            run_tag_tchannel = Output_tchannel.readlines() 

        if Cond_GluonSplit_BB == True:
            Output_gluonsplit = open('{path}/Events/Events_1/LQ_GluonS_channel_{index}/Events/run_01/run_01_tag_1_banner.txt'.format(path = path_MadGraph, index = idx), 'r')
            run_tag_gluonsplit = Output_gluonsplit.readlines() 

        if Cond_GluonSplit_BC == True:
            Output_gluonsplit_BC = open('{path}/Events/Events_1/LQ_GluonS_BC_channel_{index}/Events/run_01/run_01_tag_1_banner.txt'.format(path = path_MadGraph, index = idx), 'r')
            run_tag_gluonsplit_BC = Output_gluonsplit_BC.readlines() 

        if Cond_GluonSplit_BS == True:
            Output_gluonsplit_BS = open('{path}/Events/Events_1/LQ_GluonS_BS_channel_{index}/Events/run_01/run_01_tag_1_banner.txt'.format(path = path_MadGraph, index = idx), 'r')
    ...
    df = df.append(entry)
    df.to_csv('{path}/Events_1/Data_MadGraph_1.csv'.format(path = path_Data),sep=',',index=False)
    
    if Cond_TChannel == True:
       subprocess.call(["rm", "-rf" , "{path}/Events/Events_1/LQ_T_channel_{index}".format(path = path_MadGraph, index = idx)])
       
    if Cond_GluonSplit_BB == True:
       subprocess.call(["rm", "-rf" , "{path}/Events/Events_1/LQ_GluonS_channel_{index}".format(path = path_MadGraph, index = idx)])
       
    if Cond_GluonSplit_BC == True:
       subprocess.call(["rm", "-rf" , "{path}/Events/Events_1/LQ_GluonS_BC_channel_{index}".format(path = path_MadGraph, index = idx)])
       
    if Cond_GluonSplit_BS == True:
       subprocess.call(["rm", "-rf" , "{path}/Events/Events_1/LQ_GluonS_BS_channel_{index}".format(path = path_MadGraph, index = idx)])
